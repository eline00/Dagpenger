...
public class Dagpenger {
    ...
    public int dailyRate() {
        ...
        if (dailyBasis > 6 * G) throw new IllegalStateException("Dagpengegrunnlaget kan ikke være mer inn 6G!");

        return (int) Math.ceil(dailyBasis/workdaysPerYear);   // dagsats = dagpengegrunnlag/arbeidsdager per år rundet opp
    }
